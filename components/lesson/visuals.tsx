...
export function Formula({ parts, caption, en, ko }: { parts: string[]; caption?: string; en?: string; ko?: string }) {
  return (
    <figure className="my-5 rounded-2xl border border-line bg-card px-4 py-3.5">
      {caption && <figcaption className="mb-2 text-[13.5px] font-extrabold text-ink-3">{caption}</figcaption>}
      ...
      {ko && <p className="text-[13.5px] text-ink-2">{ko}</p>}
    </figure>
  );
}
        ...
        {items.map((it, i) => (
          <div key={it.title} className="relative flex flex-col rounded-2xl border border-line bg-card">
            {vs && i > 0 && items.length === 2 && (
              <span
                aria-hidden
                className="absolute -top-3.5 left-1/2 hidden size-8 -translate-x-1/2 place-items-center rounded-full bg-ink text-[13.5px] font-extrabold text-bg sm:-left-[1.15rem] sm:top-1/2 sm:grid sm:-translate-y-1/2 sm:translate-x-0"
              >
                vs
              </span>
            )}
            <div className={`flex items-center gap-2 rounded-t-2xl px-4 py-2.5 ${HEAD[it.tone ?? (i === 0 ? "sky" : "coral")]}`}>
              {it.mark && (
                <span className="grid size-6 shrink-0 place-items-center rounded-full bg-card text-[13.5px] font-extrabold" aria-label={it.mark === "ok" ? "맞아요" : "틀려요"}>
                  {it.mark === "ok" ? "✓" : "✕"}
                </span>
              )}
              <span className="min-w-0">
                <span className="block text-[15px] font-extrabold">{it.title}</span>
                {it.sub && <span className="block text-[13.5px] font-bold opacity-90">{it.sub}</span>}
              </span>
            </div>
            <div className="grid gap-1.5 px-4 py-3">
              ...
              {it.ko && <p className="text-[13.5px] text-ink-2">{it.ko}</p>}
              {it.points && (
        ...
        {steps.map((s, i) => (
          <li key={s.title} className="flex gap-3 rounded-2xl border border-line bg-card px-4 py-3">
            ...
            <span className="min-w-0">
              ...
              {s.ko && <span className="block text-[13.5px] text-ink-2">{s.ko}</span>}
              {s.note && <span className="mt-0.5 block text-[13.5px] text-ink-3">{s.note}</span>}
            </span>
          </li>
        ))}
      </ol>
