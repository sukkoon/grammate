...
// 전체 목차에는 제1부~제7부만 싣는다. 서장(시작하기 전에)은 /start에서 따로 다룬다
const parts = curriculum.filter((p) => p.id !== "intro");
...
export default function LearnPage() {
  const total = allUnits().filter((r) => r.part.id !== "intro").length;
  return (
...
지금 바로 시작해 보세요.`}
      ...
      <section id="map" aria-labelledby="h-map" className="mt-8 scroll-mt-24 rounded-2xl border border-line bg-card px-4 py-4 sm:px-5">
        <h2 id="h-map" className="text-[1.1rem] font-extrabold">
          한눈에 보는 핵심 문법
        </h2>
        <p className="mt-1 text-[13.5px] text-ink-2">
          <Phrases text={`7부 ${parts.reduce((n, p) => n + p.chapters.length, 0)}장이에요. 장 이름을 누르면 그 장의 단원 목록으로 내려가요.`} />
        </p>
        <ol className="mt-3 grid gap-2.5 sm:grid-cols-2">
          {parts.map((part) => (
            <li key={part.id} className="rounded-xl bg-chip px-3.5 py-3">
              <p className="text-[14.5px] font-extrabold text-coral-ink">
                {part.label} <span className="text-ink">{part.title}</span>
              </p>
              <ul className="mt-2 flex flex-wrap gap-1.5">
                {part.chapters.map((c) => (
                  <li key={c.slug}>
                    <a href={`#${c.slug}`} className="inline-block rounded-lg bg-card px-3 py-1 text-[13.5px] font-bold text-ink ring-1 ring-line hover:bg-ink hover:text-on-ink hover:ring-ink">
                      {c.title}
                    </a>
                  </li>
                ))}
        ...
        {parts.map((part) => (
          <section key={part.id} aria-labelledby={`part-${part.id}`}>
            <div className="flex items-baseline gap-3 border-b-2 border-ink pb-2">
              ...
              <h2 id={`part-${part.id}`} className="text-[1.5rem] font-extrabold">
                {part.title}
              </h2>
            </div>
