...
export default function LearnPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 pb-10 pt-10 sm:px-6">
      <h1 className="text-[1.8rem] font-extrabold tracking-[-0.02em]">전체 목차</h1>
      ...
      <p className="mt-3">
        <Link href="/roadmap" className="inline-flex items-center rounded-full bg-coral-soft px-4 py-2 text-[14.5px] font-bold text-coral-ink hover:bg-coral-soft/70">
      ...
      <section id="map" aria-labelledby="h-map" className="mt-8 scroll-mt-24 rounded-2xl border border-line bg-card px-4 py-4 sm:px-5">
        <h2 id="h-map" className="text-[1.02rem] font-extrabold">
          한눈에 보는 핵심 문법
        </h2>
        <p className="mt-1 text-[13.5px] text-ink-2">7부 {curriculum.reduce((n, p) => n + p.chapters.length, 0)}장이에요. 장 이름을 누르면 그 장의 단원 목록으로 내려가요.</p>
      ...
      <div className="mt-10 space-y-12">
        {curriculum.map((part) => (
          <section key={part.id} aria-labelledby={`part-${part.id}`}>
            <div className="flex items-baseline gap-3 border-b-2 border-ink pb-2">
              <span className="text-[13.5px] font-extrabold text-coral-ink">{part.label}</span>
              <h2 id={`part-${part.id}`} className="text-[1.35rem] font-extrabold">
                {part.title}
              </h2>
            </div>
            <div className="mt-4 space-y-6">
              {part.chapters.map((c) => (
                <div key={c.slug} id={c.slug} className="scroll-mt-24">
                  <h3 className="text-[1.08rem] font-extrabold">{c.title}</h3>
                  <p className="text-[14.5px] text-ink-2">{c.hook}</p>
                      ...
                      u.ready ? (
                        <li key={u.slug}>
                          ...
                          >
                            ...
                            <span className="mt-1 text-[13.5px] text-ink-2">{u.summary}</span>
                            <span className="mt-2">
        ...
        <section aria-labelledby="part-appendix">
          <div className="flex items-baseline gap-3 border-b-2 border-ink pb-2">
            <span className="text-[13.5px] font-extrabold text-coral-ink">부록</span>
            <h2 id="part-appendix" className="text-[1.35rem] font-extrabold">
              찾아보기
            </h2>
          </div>
            ...
            {[
              ...
              <li key={a.href}>
                <Link href={a.href} className="group flex h-full flex-col rounded-2xl border border-line bg-card px-4 py-3 transition-colors hover:border-ink-3">
                  ...
                  <span className="mt-1 text-[13.5px] text-ink-2">{a.sub}</span>
                </Link>
              </li>
            ))}
          </ul>
