...
export default function TermsPage() {
  return (
    <div className="mx-auto max-w-4xl px-4 pb-10 pt-10 sm:px-6">
      <h1 className="text-[1.8rem] font-extrabold tracking-[-0.02em]">문법 용어 사전</h1>
      ...
      <nav aria-label="용어 분류" className="mt-6 flex flex-wrap gap-2">
        {termGroups.map((g) => (
          <a key={g} href={`#g-${g}`} className="rounded-full bg-card px-3.5 py-1.5 text-[13.5px] font-bold ring-1 ring-line hover:bg-chip">
            {g}
          </a>
        ))}
      </nav>
      <div className="mt-10 space-y-12">
        {termGroups.map((g) => (
          <section key={g} id={`g-${g}`} aria-labelledby={`h-${g}`}>
            <h2 id={`h-${g}`} className="border-b-2 border-ink pb-2 text-[1.25rem] font-extrabold">
              {g}
            </h2>
                ...
                .map((t) => (
                  <article key={t.id} id={t.id} className="scroll-mt-24 rounded-2xl border border-line bg-card px-4 py-5 sm:px-6">
                    <div className="mb-3 flex items-baseline justify-between gap-3">
                      <h3 className="text-[1.35rem] font-extrabold">{t.term}</h3>
                      {t.href && (
                        <Link href={t.href} className="shrink-0 text-[13.5px] font-bold text-coral-ink hover:underline">
                          배우러 가기 ›
                        </Link>
                      )}
                    </div>
