  ...
  return (
    <div className="mx-auto max-w-4xl px-4 pb-10 pt-10 sm:px-6">
      ...
      <p className="mt-2 text-ink-2">
        <Phrases text="용어를 조각내면 뜻이 보여요. ‘-사’는 단어의 종류, ‘-어’는 문장 속 역할이에요. 영어 이름의 뜻도 함께 붙였어요." />
      </p>
      <div className="mt-5">
        <LevelTabs />
      </div>
      <nav aria-label="용어 분류" className="mt-6 flex flex-wrap gap-2">
        {termGroups.map((g) => (
          <a key={g} href={`#g-${g}`} className="rounded-lg bg-card px-3.5 py-1.5 text-[13.5px] font-bold ring-1 ring-line hover:bg-chip">
            {g}
          </a>
        ))}
      </nav>
                ...
                .map((t) => (
                  <article key={t.id} id={t.id} className="scroll-mt-24 rounded-2xl border border-line bg-card px-4 py-5 sm:px-6">
                    <div className="mb-3 flex items-baseline justify-between gap-3">
                      <h3 className="text-[1.5rem] font-extrabold">{t.term}</h3>
                      {t.href && (
