...
export default function VerbsPage() {
  return (
    <div className="mx-auto max-w-3xl px-4 pb-10 pt-10 sm:px-6">
      <p className="text-[14px] font-extrabold text-coral-ink">부록</p>
      <h1 className="mt-1 text-[2rem] font-extrabold tracking-[-0.02em]">불규칙 동사표</h1>
      ...
      <section aria-labelledby="h-groups" className="mt-8">
        <h2 id="h-groups" className="text-[1.25rem] font-extrabold">
          네 가지 모양으로 묶어 외워요
        </h2>
          ...
          {GROUP_ORDER.map((g) => (
            <li key={g} className="rounded-2xl border border-line bg-card px-4 py-3">
              <p className="flex items-baseline justify-between gap-2">
                ...
                <span className="text-[14px] font-bold text-ink-3">{irregularVerbs.filter((v) => v.group === g).length}개</span>
              </p>
              <p className="text-[15px] text-ink-2">{groupInfo[g].desc}</p>
      ...
      <section aria-labelledby="h-table" className="mt-10">
        <h2 id="h-table" className="text-[1.25rem] font-extrabold">
          찾고, 가리고, 들어 봐요
        </h2>
      ...
      <section aria-labelledby="h-confusing" className="mt-12">
        <h2 id="h-confusing" className="flex flex-wrap items-center gap-2 text-[1.25rem] font-extrabold">
          모양이 닮아서 헷갈리는 동사
          <span className="rounded-full bg-sky-soft px-2.5 py-0.5 text-[14px] font-extrabold text-sky-ink">고등 어법</span>
        </h2>
              ...
              <ul className="grid gap-1.5">
                {c.rows.map((r) => (
                  <li key={r.forms} className="flex flex-wrap items-baseline gap-x-3">
                    ...
                    <span className="text-[15px] text-ink-2">{r.ko}</span>
                  </li>
                ))}
              </ul>
              <p className="mt-2 flex items-start gap-2 rounded-xl bg-chip px-3 py-2 text-[14.5px]">
